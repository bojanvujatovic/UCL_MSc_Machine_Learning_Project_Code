...
do
    ...
    do
		...
		do
                    # Create directory
                    dir_name=$(printf "experiment-%s-%s-%s-%s-%s-%s" $phenotype $percentage $model "0" "0" $i)
                    mkdir $dir_name
					...
					python create_random_subsample.py helper_files/keep-100 helper_files/random-subsample				

                    plink --noweb \
                          --bfile data-filtered \
                          --pheno helper_files/$phenotype \
                          --recode \
                          --keep helper_files/random-subsample \
